...
def limpiar_busqueda(query):
    ...
    palabras_basura = {'video', 'oficial', 'official', 'lyric', 'letra', 'audio', 'full', 'hd'}
        ...
        if cola_musica and not pygame.mixer.music.get_busy() and not getattr(gui, 'musica_pausada', False):
            ...
            if stream_url:
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=15) as response, open(archivo_temp, 'wb') as f:
                        while True:
                    ...
                    if os.path.exists(archivo_temp) and os.path.getsize(archivo_temp) > 10000:
                        ...
                        gui.agregar_log(f"[REPRODUCIENDO] {cancion_actual}")
...
class PanelControl:
    def __init__(self):
        ...
        self.entry_cmd_stop = _crear_campo_cmd(frame_cmd_cfg, "Stop/Parar:", "cmd_stop")
        ...
        self.entry_cmd_song = _crear_campo_cmd(frame_cmd_cfg, "Canción Actual:", "cmd_song")
    ...
    def alternar_pausa_musica(self):
        ...
        elif pygame.mixer.music.get_busy() or cancion_actual:
    ...
    def saltar_cancion_manual(self):
        global cancion_actual
        self.musica_pausada = False
        if pygame.mixer.music.get_busy() or cancion_actual:
            pygame.mixer.music.stop()
            pygame.mixer.music.unload()
    ...
    def al_cerrar(self):
        ...
        datos_guardar = {
            ...
            "cmd_stop": self.entry_cmd_stop.get().strip(),
            ...
            "cmd_song": self.entry_cmd_song.get().strip(),
...
def extraer_o_limpiar_emojis(texto, max_emojis):
    """
    Conserva hasta `max_emojis` emojis en el texto y elimina los sobrantes.
    Convierte fuentes Unicode estilizadas a texto legible estándar sin librerías externas.
    """
...
def normalizar_texto(texto):
    """Mantiene compatibilidad con comandos de voz quitando todos los emojis."""
    ...
    while True:
        ...
        try:
            if not gui.audio_pausado:
                ...
                if canal_tts:
                    canal_tts.set_volume(float(gui.slider_volumen.get()))
...
def procesar_comandos_musica(comentario, username):
    ...
    cmds_stop = gui.obtener_lista_comandos(gui.entry_cmd_stop)
    ...
    cmds_song = gui.obtener_lista_comandos(gui.entry_cmd_song)

    if comando in cmds_play:
        if not arg:
            enviar_a_voz("Escribe el nombre de la canción después de play")
            return True
        cola_musica.append((arg, nombre_user))
        gui.actualizar_lista_musica_ui()
        gui.agregar_log(f"[MÚSQUEDA] @{nombre_user} añadió a la cola: {arg}")
        enviar_a_voz(f"Canción añadida a la cola por {nombre_user}")
        return True
        ...
        if pygame.mixer.music.get_busy() or cancion_actual:
            pygame.mixer.music.stop()
            pygame.mixer.music.unload()
            gui.agregar_log(f"[MÚSQUEDA] @{nombre_user} saltó la canción")
            enviar_a_voz("Canción saltada")
        else:
            enviar_a_voz("No hay ninguna canción sonando")
        return True

    elif comando in cmds_stop:
        gui.musica_pausada = False
        cola_musica.clear()
        gui.actualizar_lista_musica_ui()
        pygame.mixer.music.stop()
        ...
        cancion_actual = None
        gui.actualizar_cancion_actual_ui("Ninguna")
        gui.agregar_log(f"[MÚSQUEDA] @{nombre_user} detuvo la música y vació la cola")
        enviar_a_voz("Música deteniéndose")
        return True
    ...
    elif comando in cmds_song:
        if cancion_actual:
            enviar_a_voz(f"Sonando ahora: {cancion_actual}")
        else:
            enviar_a_voz("No hay canción sonando")
        return True
    ...
    try:
        ...
        async def on_comment(event: CommentEvent):
            ...
            if comentario.startswith("!"):
                if procesar_comandos_musica(comentario, nickname or username):
                    return
            ...
            if permitido:
                ...
                # Máximo 1 emoji en el nombre del usuario
                ...
                # Permite máximo 3 emojis en el texto del comentario
        ...
        async def on_gift(event: GiftEvent):
            ...
            gui.agregar_log(f"[REGALO] {nombre_limpio} envió {regalo}")
        ...
        async def on_like(event: LikeEvent):
            ...
            if gui.alerta_likes_persona.get():
                ...
                if LIKES_POR_USUARIO[username] >= meta_persona:
                    ...
                    enviar_a_voz(f"¡Gracias {nombre_limpio} por los {meta_persona} likes!")
