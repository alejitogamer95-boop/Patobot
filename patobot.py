        ...
        for motor in motores:
            try:
                ...
                if stream_url:
                    ...
                    return stream_url, titulo_cancion, info.get("thumbnail", "")
...
def reproductor_musica_loop():
    global cancion_actual, VOTOS_SKIP, canal_musica_ram, CANCION_ACTUAL_WIDGET
    ...
    while True:
        ...
        if cola_musica and not esta_ocupado and not getattr(gui, 'musica_pausada', False):
            ...
            stream_url, titulo, thumbnail = obtener_stream_audio(query)
            if stream_url:
                try:
                    cancion_actual = f"{titulo} (Pedida por @{usuario})"
                    CANCION_ACTUAL_WIDGET = {"title": titulo, "user": usuario, "duration": 0, "started_at": 0, "cover": thumbnail or ""}
                    VOTOS_SKIP.clear()
                    ...
                    if len(buffer_ram.getvalue()) > 10000:
                        ...
                        CANCION_ACTUAL_WIDGET["duration"] = max(0, float(sonido_cancion.get_length()))
                        CANCION_ACTUAL_WIDGET["started_at"] = time.time()
                        broadcast_overlay_data()
...
class PanelControl:
    ...
    def buscar_cancion_desde_ui(self):
        """Añade una canción escrita en la barra de búsqueda a la cola."""
        global cancion_actual

        query = self.entry_busqueda_musica.get().strip()
        if not query:
            self.agregar_log("[MÚSICA] Escribe el nombre de una canción para buscar.")
            return

        arg_normalizado = query.lower()
        if cancion_actual and arg_normalizado in cancion_actual.lower():
            self.agregar_log(f"[MÚSICA] '{query}' ya se está reproduciendo.")
            return

        ya_en_cola = any(q.strip().lower() == arg_normalizado for q, _ in cola_musica)
        if ya_en_cola:
            self.agregar_log(f"[MÚSICA] '{query}' ya se encuentra en la cola.")
            return

        cola_musica.append((query, "Dashboard"))
        self.actualizar_lista_musica_ui()
        self.agregar_log(f"[MÚSICA] Añadida desde búsqueda: {query}")
