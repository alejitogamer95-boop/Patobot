...
class SpotifyManager:
    """Spotify OAuth + Web API manager for PatoBot."""
    def __init__(self):
    ...
    def load_from_config(self):
        # config is defined later; this is intentionally defensive.
        try:
    ...
    def start_auth(self):
        if not self.client_id:
            raise RuntimeError("Configura primero el Spotify Client ID.")
    ...
    def _ensure_token(self):
        ...
        if time.time() >= self.expires_at and not self.refresh():
            raise RuntimeError("La sesión de Spotify expiró; vuelve a conectar.")
    ...
    def api(self, method, path, params=None, body=None, retry=True):
        ...
        try:
            ...
            if e.code == 401 and retry and self.refresh():
                return self.api(method, path, params, body, retry=False)
            try:
    ...
    def search_track(self, query):
        ...
        if not clean:
            return None
        data = self.api("GET", "/search", params={"q": clean, "type": "track", "limit": 5})
        items = ((data or {}).get("tracks") or {}).get("items") or []
        if not items:
            return None
        # Prefer an exact artist/title match when the viewer supplied "title artist".
        qnorm = limpiar_busqueda(clean)
        best = items[0]
        best_score = -1
        for item in items:
            title = item.get("name", "")
            artists = ", ".join(a.get("name", "") for a in item.get("artists", []))
            candidate = limpiar_busqueda(f"{title} {artists}")
            score = 0
            qwords = set(qnorm.split())
            cwords = set(candidate.split())
            score += len(qwords & cwords) * 10
            if qnorm == candidate:
                score += 100
            if score > best_score:
                best_score = score
        ...
        return {
            ...
            "cover": ((best.get("album") or {}).get("images") or [{}])[0].get("url", ""),
            "duration": (best.get("duration_ms") or 0) / 1000.0,
            "url": ((best.get("external_urls") or {}).get("spotify", "")),
        }

    def devices(self):
        data = self.api("GET", "/me/player/devices") or {}
        return data.get("devices", [])

    def choose_device(self, preferred_id=None):
        devices = self.devices()
        if not devices:
            raise RuntimeError("No hay dispositivos Spotify disponibles. Abre Spotify en tu PC/celular/altavoz.")
        chosen = None
        if preferred_id:
            chosen = next((d for d in devices if d.get("id") == preferred_id), None)
        if not chosen:
            chosen = next((d for d in devices if d.get("is_active")), None)
        if not chosen:
            chosen = next((d for d in devices if not d.get("is_restricted")), devices[0])
        self.device_id = chosen.get("id")
        self.device_name = chosen.get("name", "")
        self.device_type = chosen.get("type", "")
        self._save_tokens()
        return chosen

    def playback(self):
        return self.api("GET", "/me/player")

    def play_track(self, track):
        if not self.device_id:
            self.choose_device()
        body = {"uris": [track["uri"]]}
        self.api("PUT", "/me/player/play", params={"device_id": self.device_id}, body=body)

    def pause(self):
        if not self.device_id:
            self.choose_device()
        self.api("PUT", "/me/player/pause", params={"device_id": self.device_id})

    def resume(self):
        if not self.device_id:
            self.choose_device()
        self.api("PUT", "/me/player/play", params={"device_id": self.device_id})

    def skip(self):
        if not self.device_id:
            self.choose_device()
        self.api("POST", "/me/player/next", params={"device_id": self.device_id})

    def volume(self, percent):
        if not self.device_id:
            self.choose_device()
        value = max(0, min(100, int(percent)))
        self.api("PUT", "/me/player/volume", params={"device_id": self.device_id, "volume_percent": value})
...
def spotify_login():
    try:
        ...
        return f"<html><body style='font-family:Segoe UI;background:#111;color:#fff;padding:40px'><h2>Spotify OAuth</h2><p>Abriendo Spotify para autorizar PatoBot...</p><p>Si no se abrió, <a style='color:#1db954' href='{url}'>haz clic aquí</a>.</p></body></html>"
    except Exception as e:
        return f"<h2>Error Spotify</h2><pre>{str(e)}</pre>", 400

@app.route("/spotify/callback")
def spotify_callback():
    error = request.args.get("error")
    if error:
        return f"<html><body style='font-family:Segoe UI;padding:40px'><h2>Spotify rechazó la autorización</h2><p>{error}</p></body></html>", 400
    try:
        spotify.finish_auth(request.args.get("code", ""), request.args.get("state", ""))
        try:
            spotify.choose_device()
        except Exception:
            pass
        try:
            gui.actualizar_estado_spotify()
        except Exception:
            pass
        return "<html><body style='font-family:Segoe UI;background:#111;color:#fff;padding:40px'><h2 style='color:#1db954'>✓ Spotify conectado</h2><p>Ya puedes cerrar esta ventana y volver a PatoBot.</p></body></html>"
    except Exception as e:
        return f"<html><body style='font-family:Segoe UI;padding:40px'><h2>Error OAuth</h2><pre>{str(e)}</pre></body></html>", 400
...
def broadcast_overlay_data():
    ...
    queue_payload = [{"query": x.get("query", ""), "title": x.get("title", x.get("query", "")), "artist": x.get("artist", ""), "user": x.get("user", "Usuario"), "cover": x.get("cover", ""), "uri": x.get("uri", "")} for x in list(cola_musica)]
...
cola_musica = deque()
cancion_actual = None
canal_musica_ram = None  # Se conserva para TTS/compatibilidad; la música ahora la controla Spotify.

# Información de la canción que PatoBot está reproduciendo en Spotify.
SPOTIFY_CURRENT_REQUEST = None
SPOTIFY_LAST_PLAYBACK = None
SPOTIFY_MUSIC_STOP = threading.Event()


def spotify_enqueue(query, usuario):
    """Busca una canción en Spotify y la deja en la cola propia de PatoBot."""
    if not spotify.access_token:
        raise RuntimeError("Spotify no está conectado. Usa 'Conectar Spotify' en Música y Comandos.")
    track = spotify.search_track(query)
    if not track:
        raise RuntimeError(f"No encontré '{query}' en Spotify.")
    cola_musica.append({
        "query": query,
        "user": usuario,
        "title": track["title"],
    ...
    try:
        ...
        gui.agregar_log(f"[SPOTIFY] @{usuario} añadió: {track['title']} — {track['artist']}")
    except Exception:
        pass
    return track


def spotify_start_next():
    global SPOTIFY_CURRENT_REQUEST, cancion_actual, CANCION_ACTUAL_WIDGET
    if not cola_musica:
        return False
    if not spotify.access_token:
        return False
    ...
    try:
        spotify.play_track(item)
    except Exception as e:
        # Si el dispositivo guardado desapareció, localizar uno nuevo y reintentar.
        try:
            spotify.choose_device()
            spotify.play_track(item)
        except Exception as e2:
            cola_musica.appendleft(item)
            try: gui.agregar_log(f"[SPOTIFY] No se pudo reproducir: {e2}")
            except Exception: pass
            return False
    SPOTIFY_CURRENT_REQUEST = item
    cancion_actual = f"{item['title']} — {item['artist']} (Pedida por @{item['user']})"
    CANCION_ACTUAL_WIDGET = {
        "title": item["title"],
        "artist": item["artist"],
        "user": item["user"],
    ...
    try: gui.actualizar_lista_musica_ui(); gui.actualizar_cancion_actual_ui(cancion_actual)
    except Exception: pass
    broadcast_overlay_data()
    return True


def sincronizar_spotify():
    """Sincroniza estado real de Spotify y avanza automáticamente la cola de PatoBot."""
    global SPOTIFY_LAST_PLAYBACK, SPOTIFY_CURRENT_REQUEST, cancion_actual, CANCION_ACTUAL_WIDGET
    while not SPOTIFY_MUSIC_STOP.is_set():
        try:
            if spotify.access_token:
                state = spotify.playback()
                SPOTIFY_LAST_PLAYBACK = state
                item = (state or {}).get("item") if state else None
                is_playing = bool((state or {}).get("is_playing"))
                progress_ms = int((state or {}).get("progress_ms") or 0)
                device = (state or {}).get("device") or {}
                if device.get("id"):
                    spotify.device_id = device.get("id")
                    spotify.device_name = device.get("name", spotify.device_name)
                    spotify.device_type = device.get("type", spotify.device_type)

                if item and item.get("type") == "track":
                    current_uri = item.get("uri", "")
                    duration_ms = int(item.get("duration_ms") or 0)
                    # Una pista que llegó al final deja de estar en playing pero conserva item/progress.
                    if (not is_playing and duration_ms > 0 and progress_ms >= max(0, duration_ms - 1200)
                            and not getattr(gui, "musica_pausada", False)):
                        if cola_musica:
                            SPOTIFY_CURRENT_REQUEST = None
                            spotify_start_next()
                            time.sleep(0.15)
                            ...
                            SPOTIFY_CURRENT_REQUEST = None
                    requested = SPOTIFY_CURRENT_REQUEST
                    # Si Spotify cambió de canción por fuera del bot, reflejarla en el widget.
                    if not requested or requested.get("uri") != current_uri:
                        CANCION_ACTUAL_WIDGET = {
                            "title": item.get("name", ""),
                            "artist": ", ".join(a.get("name", "") for a in item.get("artists", [])),
                            "user": requested.get("user", "Spotify") if requested and requested.get("uri") == current_uri else "Spotify",
                            "duration": (item.get("duration_ms") or 0) / 1000.0,
                            "started_at": time.time() - progress_ms / 1000.0,
                        ...
                        CANCION_ACTUAL_WIDGET["progress_ms"] = progress_ms
                        CANCION_ACTUAL_WIDGET["started_at"] = time.time() - progress_ms / 1000.0
                        CANCION_ACTUAL_WIDGET["paused"] = not is_playing
                else:
                    # Sin reproducción: solo arrancamos la siguiente si no estamos pausados.
                    if not getattr(gui, "musica_pausada", False) and cola_musica:
                        spotify_start_next()
                    elif not is_playing and SPOTIFY_CURRENT_REQUEST and progress_ms == 0:
                        SPOTIFY_CURRENT_REQUEST = None
                        cancion_actual = None
                        CANCION_ACTUAL_WIDGET = {"title":"", "user":"", "duration":0, "started_at":0, "cover":""}

                try: gui.actualizar_estado_spotify()
                except Exception: pass
                broadcast_overlay_data()
        except Exception as e:
            try: gui.agregar_log(f"[SPOTIFY] {e}")
            except Exception: pass
        time.sleep(1.2)

threading.Thread(target=sincronizar_spotify, daemon=True).start()
...
class PanelControl:
    def __init__(self):
        ...
        self.spotify_status_var = tk.StringVar(value="Spotify: desconectado")
        ...
        # Tab Música
        # Integración Spotify OAuth + Web API
        frame_spotify = ttk.LabelFrame(self.tab_musica, text=" Spotify OAuth / Web API ")
        frame_spotify.pack(fill="x", padx=10, pady=5)
        f_sp1 = ttk.Frame(frame_spotify); f_sp1.pack(fill="x", padx=8, pady=4)
        ttk.Label(f_sp1, text="Client ID:", width=12).pack(side="left")
        self.entry_spotify_client_id = tk.Entry(f_sp1, bg="#11111b", fg="#cdd6f4", insertbackground="white", relief="flat")
        self.entry_spotify_client_id.insert(0, config.get("spotify_client_id", ""))
        self.entry_spotify_client_id.pack(side="left", fill="x", expand=True, padx=5)
        f_sp2 = ttk.Frame(frame_spotify); f_sp2.pack(fill="x", padx=8, pady=4)
        ttk.Label(f_sp2, text="Client Secret:", width=12).pack(side="left")
        self.entry_spotify_client_secret = tk.Entry(f_sp2, show="*", bg="#11111b", fg="#cdd6f4", insertbackground="white", relief="flat")
        self.entry_spotify_client_secret.insert(0, config.get("spotify_client_secret", ""))
        self.entry_spotify_client_secret.pack(side="left", fill="x", expand=True, padx=5)
        f_sp3 = ttk.Frame(frame_spotify); f_sp3.pack(fill="x", padx=8, pady=4)
        ttk.Label(f_sp3, textvariable=self.spotify_status_var).pack(side="left", fill="x", expand=True)
        self.btn_spotify_login = tk.Button(f_sp3, text="🎵 Conectar Spotify", bg="#1db954", fg="white", relief="flat", command=self.conectar_spotify, font=(self.fuente_actual, 8, "bold"))
        self.btn_spotify_login.pack(side="left", padx=3)
        self.btn_spotify_device = tk.Button(f_sp3, text="Dispositivo", bg="#89b4fa", fg="#11111b", relief="flat", command=self.seleccionar_dispositivo_spotify, font=(self.fuente_actual, 8, "bold"))
        self.btn_spotify_device.pack(side="left", padx=3)
        self.btn_spotify_logout = tk.Button(f_sp3, text="Desconectar", bg="#f38ba8", fg="#11111b", relief="flat", command=self.desconectar_spotify, font=(self.fuente_actual, 8, "bold"))
        self.btn_spotify_logout.pack(side="left", padx=3)
        ttk.Label(frame_spotify, text="Redirect URI: http://127.0.0.1:5000/spotify/callback").pack(anchor="w", padx=8, pady=(0,5))
    ...
    def cambiar_volumen_musica(self, val):
        try:
            value = max(0, min(100, int(float(val) * 100)))
            spotify.volume(value)
        except Exception:
            # El slider también se puede mover antes de conectar Spotify.
            pass
    ...
    def alternar_pausa_musica(self):
        ...
        try:
            if self.musica_pausada:
                spotify.resume()
                self.musica_pausada = False
                self.agregar_log("[SPOTIFY] Música reanudada manualmente.")
            else:
                spotify.pause()
                self.musica_pausada = True
                self.agregar_log("[SPOTIFY] Música pausada manualmente.")
            self.actualizar_estado_spotify()
        except Exception as e:
            self.agregar_log(f"[SPOTIFY] Error pausa/reanudar: {e}")
    ...
    def saltar_cancion_manual(self):
        ...
        try:
            if cola_musica:
                # Para una solicitud controlada por PatoBot, reproducimos directamente la siguiente.
                spotify_start_next()
            else:
                spotify.skip()
                SPOTIFY_CURRENT_REQUEST = None
                self.agregar_log("[SPOTIFY] Canción saltada desde el Dashboard.")
            VOTOS_SKIP.clear()
        except Exception as e:
            self.agregar_log(f"[SPOTIFY] Error al saltar: {e}")

    def conectar_spotify(self):
        try:
            client_id = self.entry_spotify_client_id.get().strip()
            client_secret = self.entry_spotify_client_secret.get().strip()
            if not client_id or not client_secret:
                self.agregar_log("[SPOTIFY] Introduce Client ID y Client Secret.")
                return
            spotify.configure(client_id, client_secret)
            config["spotify_client_id"] = client_id
            config["spotify_client_secret"] = client_secret
            guardar_configuracion(config)
            spotify.start_auth()
            self.spotify_status_var.set("Spotify: esperando autorización…")
            self.agregar_log("[SPOTIFY] Se abrió el navegador para autorizar la cuenta.")
        except Exception as e:
            self.agregar_log(f"[SPOTIFY] Error OAuth: {e}")

    def seleccionar_dispositivo_spotify(self):
        try:
            devices = spotify.devices()
            if not devices:
                raise RuntimeError("No hay dispositivos disponibles. Abre Spotify primero.")
            active = next((d for d in devices if d.get("is_active")), devices[0])
            spotify.device_id = active.get("id")
            spotify.device_name = active.get("name", "")
            spotify.device_type = active.get("type", "")
            spotify._save_tokens()
            self.actualizar_estado_spotify()
            self.agregar_log(f"[SPOTIFY] Dispositivo seleccionado: {spotify.device_name}")
        except Exception as e:
            self.agregar_log(f"[SPOTIFY] Error dispositivo: {e}")

    def desconectar_spotify(self):
        spotify.clear_session()
        self.spotify_status_var.set("Spotify: desconectado")
        self.agregar_log("[SPOTIFY] Sesión desconectada.")

    def actualizar_estado_spotify(self):
        def _update():
            if spotify.access_token:
                device = spotify.device_name or "sin dispositivo"
                self.spotify_status_var.set(f"Spotify: conectado · {device}")
            else:
                self.spotify_status_var.set("Spotify: desconectado")
    ...
    def buscar_cancion_desde_ui(self):
        ...
        if not query:
            self.agregar_log("[SPOTIFY] Escribe el nombre de una canción.")
            return
        try:
            spotify_enqueue(query, "Dashboard")
            self.entry_busqueda_musica.delete(0, tk.END)
            # Si no hay nada sonando, iniciar inmediatamente.
            state = spotify.playback()
            if not state or not state.get("item") or not state.get("is_playing"):
                spotify_start_next()
        except Exception as e:
            self.agregar_log(f"[SPOTIFY] {e}")
    ...
    def actualizar_lista_musica_ui(self):
        def _update():
            try:
                ...
                for idx, item in enumerate(list(cola_musica), start=1):
                    title = item.get("title", item.get("query", "Canción"))
                    artist = item.get("artist", "")
                    usuario = item.get("user", "Usuario")
                    texto = f"{idx}. {title}"
                    if artist: texto += f" — {artist}"
                    texto += f" (por @{usuario})"
                    self.listbox_musica.insert(tk.END, texto)
    ...
    def al_cerrar(self):
        ...
        datos_guardar = {
            ...
            "spotify_client_id": self.entry_spotify_client_id.get().strip(),
            "spotify_client_secret": self.entry_spotify_client_secret.get().strip(),
            ...
            "spotify_device_id": spotify.device_id or "",
            "spotify_device_name": spotify.device_name,
            "spotify_device_type": spotify.device_type,
...
def procesar_comandos_musica(comentario, username, user_obj):
    global VOTOS_SKIP, ULTIMO_SKIP_TIEMPO
    ...
    if comando in cmds_play:
        ...
        if not arg:
            return True
        try:
            # Busca y guarda metadatos reales de Spotify antes de añadir a la cola.
            normal = limpiar_busqueda(arg)
            current = CANCION_ACTUAL_WIDGET.get("title", "") + " " + CANCION_ACTUAL_WIDGET.get("artist", "")
            if current.strip() and normal == limpiar_busqueda(current):
                gui.agregar_log(f"[SPOTIFY] @{nombre_user} esa canción ya está sonando.")
                return True
            for item in cola_musica:
                if limpiar_busqueda(item.get("title", "") + " " + item.get("artist", "")) == normal:
                    gui.agregar_log(f"[SPOTIFY] @{nombre_user} esa canción ya está en la cola.")
                    return True
            spotify_enqueue(arg, nombre_user)
            # Primera solicitud: empieza inmediatamente si Spotify está libre.
            try:
                state = spotify.playback()
            except Exception:
                state = None
        ...
        if tiempo_actual - ULTIMO_SKIP_TIEMPO < COOLDOWN_SKIP_SEGUNDOS:
            gui.agregar_log("[SPOTIFY] Espera unos segundos antes de otro !skip.")
            return True
        ...
        if es_mod_o_dj:
            try:
                ...
                gui.agregar_log(f"[SPOTIFY] Error !skip: {e}")
            return True
        ...
        if conteo_votos >= UMBRAL_VOTOS_SKIP:
            try:
                ...
                gui.agregar_log(f"[SPOTIFY] Error al saltar: {e}")
            VOTOS_SKIP.clear()
            ULTIMO_SKIP_TIEMPO = tiempo_actual
        return True
        ...
        if not tiene_permiso_comando(user_obj, "pause"):
            gui.agregar_log(f"[SPOTIFY] @{nombre_user} sin permisos para !pause")
            return True
        try:
            spotify.pause(); gui.musica_pausada = True
            gui.agregar_log(f"[SPOTIFY] @{nombre_user} pausó la música.")
        except Exception as e: gui.agregar_log(f"[SPOTIFY] {e}")
        return True

    elif comando in cmds_resume:
        if not tiene_permiso_comando(user_obj, "resume"):
            gui.agregar_log(f"[SPOTIFY] @{nombre_user} sin permisos para !resume")
            return True
        try:
            spotify.resume(); gui.musica_pausada = False
        ...
        try:
            ...
            gui.slider_volumen_musica.set(val / 100.0)
            spotify.volume(val)
            gui.agregar_log(f"[SPOTIFY] Volumen cambiado a {val}%")
        except Exception as e:
            gui.agregar_log(f"[SPOTIFY] Volumen inválido: {e}")
        return True
